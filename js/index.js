...
const trackListElement = document.getElementById('mymusicset');
...
function loadTrack(track_index) {
    ...
    playpause_btn.innerHTML = '<i class="material-icons">play_arrow</i>'
    ...
    function playpauseTrack() {
      ...
      function playTrack() {
        ...
        playpause_btn.innerHTML = '<i class="material-icons">pause</i>';
      }
      ...
      function myFunction() {
        document.getElementById("mymusicset").classList.toggle("show");
      }
      
      // Закройте выпадающее меню, если пользователь щелкает за его пределами
      window.onclick = function(event) {
        if (!event.target.matches('.musicset')) {
          var musiclset = document.getElementsByClassName("musicset-content");
          var i;
          for (i = 0; i < musiclset.length; i++) {
            var openmusicset = musiclset[i];
            if (openmusicset.classList.contains('show')) {
              openmusicset.classList.remove('show');
            }
          }
        }
      }
